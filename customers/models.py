...
class Customer(models.Model):
    ...
    customer_type = models.CharField(max_length=11, choices=CUSTOMER_CHOICES)  
    # if customer is B2C
    visa_type = models.CharField(max_length=66, choices=RESIDENT_CHOICES, null=True, blank=True)                #resident with EmiratesID/ GCC national /tourist 
    ...
    trafficCode = models.CharField(max_length=20, unique=True, null=True, blank=True)                                            #B2B customers
    ...
    def clean(self):
        if self.customer_type == 'B2C' and not self.id_card_no:
            raise ValidationError(_('B2C customers must provide ID card number.'))
        if self.customer_type == 'B2B' and not self.tradeLicence:
            raise ValidationError(_('B2B customers must provide Trade Licence.'))
